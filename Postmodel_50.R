...
tover1<-Net_with_structure_supp     #USdata_50
tover1<-na.omit(tover1)
tover<-as.numeric(tover1)




Vis.mat<-tover1[,c(1,7,13)]
names(Vis.mat)<-c("Early","Mid","Late")

Vis.mat<-reshape2::melt(Vis.mat,value.name = "Turnover",variable.name = "Period")
...
maineffects.glm1 <-glm.cons(formula = Vis.bc1 ~bet.bc1+Enc.bc1+XP.bc1+XA.bc1+Fi.bc1, 
                            data = tover1,cons = 1,na.action = na.pass)
...
for(n in 1:length(resp_bc1_name)){
  ...
  g.l.m1<-glm(formula = f, data=tover1, family = quasibinomial(link = 'logit'))
...
for(n in 1:length(resp_bc2_name)){
  ...
  g.l.m2<-glm(formula = f, data=tover1, family = quasibinomial(link = 'logit'))
...
for(n in 1:length(resp_bc3_name)){
  ...
  g.l.m3<-glm(formula = f, data=tover1, family = quasibinomial(link = 'logit'))
...
#facet_grid(Phase ~ .)
Turnv.plot
ggsave("Turnv_US50.jpeg", plot =Turnv.plot ,
...
tplot<-boxplot(tover1[,c(1,7,13)],col = c("grey"), boxwex = 0.5, ylab="Bray-Curtis turnover", 
...
boxplot(tover1[,7:12],col = c("grey",rep("white",5)), boxwex = 0.5, ylab="Bray-Curtis turnover", 
        main="Mid phase", names =c("Int.","FE","Enc.","Pl.","An.","Fl."),
        ylim = c(0, 1),cex.lab=2.0,cex.axis=2.0,cex.main=2.0 )
abline(v=c(1.5),lty=2)

boxplot(tover1[,1:6],col = c("grey",rep("white",5)), boxwex = 0.5, ylab="Bray-Curtis turnover", 
        main="Early phase", names =c("Int.","FE","Enc.","Pl.","An.","Fl."),
        ylim = c(0, 1),cex.lab=2.0,cex.axis=2.0,cex.main=2.0 )
abline(v=c(1.5),lty=2)

boxplot(tover1[,13:18],col = c("grey",rep("white",5)), boxwex = 0.5, ylab="Bray-Curtis turnover", 
        main="Late phase", names =c("Int.","FE","Enc.","Pl.","An.","Fl."),
        ylim = c(0, 1),cex.lab=2.0,cex.axis=2.0,cex.main=2.0 )
abline(v=c(1.5),lty=2)


dev.copy(jpeg,"toverfullS50.jpeg",width = 300, height = 300,units = "mm", res = 600)
dev.off()
...
cor_dat<-tover1[c(1,7,13,20,21,26:30,33,35:37,39,41,42)]
...
ggsave("USggsig50.jpeg", plot =USggsigplot ,
